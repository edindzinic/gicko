export type Language = "en" | "bs" | "de";

export const LANGUAGE_OPTIONS: { code: Language; label: string }[] = [
  { code: "en", label: "English" },
  { code: "bs", label: "Bosanski" },
  { code: "de", label: "Deutsch" },
];

const en = {
  nav: {
    home: "Home",
    calendar: "Calendar",
    dashboard: "Dashboards",
    settings: "Settings",
  },
  common: {
    cancel: "Cancel",
    save: "Save",
    saving: "Saving…",
    loading: "Loading…",
    no: "No",
    yesDelete: "Yes, delete",
    notesOptional: "Notes (optional)",
    none: "None",
    previous: "Previous",
    next: "Next",
    today: "Today",
  },
  actions: {
    logSleep: "😴 Log sleep",
    logFeeding: "🍼 Log feeding",
    logNightWaking: "🌙 Log night waking",
  },
  home: {
    previousDay: "Previous day",
    nextDay: "Next day",
    today: "Today",
    asleepSince: "Asleep since",
    awakeSince: "Awake since",
    asleep: "asleep",
    awake: "awake",
    nightAwakening: "🌙 Night waking",
    wokeUp: "😴 Woke up",
    morningWake: "☀️ Morning wake",
    putDownToSleep: "🌙 Put down to sleep",
    logAFeeding: "🍼 Log a feeding",
    statNightSleep: "Night sleep",
    statDaytimeAwake: "Daytime awake",
    statNapsTotal: "Naps total",
    statFeedings: "Feedings",
    statEaten: "Eaten",
    statNightWakeUps: "Night wake-ups",
    statPoops: "Poops",
    timeline: "Timeline",
    feedingsToday: "Feedings today",
    noFeedingsToday: "No feedings logged today.",
    wakeUpsToday: "Night wake-ups today",
    noWakeUpsToday: "No night wake-ups today.",
    awakeFor: (d: string) => `Awake for ${d}`,
    napsToday: "Naps today",
    noNapsToday: "No naps logged today.",
    logAPoop: "💩 Poop",
    poopsToday: "Poops today",
    noPoopsToday: "No poops logged today.",
    addPoop: "Add one",
    removePoop: "Remove one",
    ongoing: "ongoing",
    wakePromptQuestion: "Did he eat during that wake-up?",
    logIt: "Log it",
    nextNap: "Next nap",
    nextBedtime: "Next bedtime",
    expectedWakeUp: "Expected wake-up",
  },
  calendar: {
    month: "Month",
    week: "Week",
    weekdayLetters: ["M", "T", "W", "T", "F", "S", "S"],
  },
  // Word for word what the push notification says, so seeing both is never confusing.
  reminders: {
    nap_due: { title: "Nap in 10 minutes", body: "The wake window is nearly up." },
    bedtime_due: { title: "Time for a bath", body: "Bedtime is about twenty minutes away." },
    nap_end: { title: "Wake him in 5 minutes", body: "The nap is almost as long as it should be." },
    feeding_due: { title: "Time for a feeding", body: "An hour and a half since the last one." },
    dismiss: "Dismiss",
  },
  activity: {
    title: "Lately",
    open: "What the others logged",
    empty: "Nobody else has logged anything today.",
    nap: "Nap",
    nightSleep: "Night sleep",
    nightWaking: "Night waking",
    poop: "Poop",
    markAllRead: "Mark all as read",
  },
  dashboard: {
    title: "Dashboards",
    napsCount: "Number of naps",
    wakeUpLength: "Avg night waking length",
    feedingsMl: "Total ml eaten",
    average: "Avg",
    noData: "No data in this range.",
  },
  dayDetail: {
    note: "Note",
    notePlaceholder: "Anything worth remembering about this day…",
  },
  timelineView: {
    nightSleep: "Night sleep",
    nap: "Nap",
    nightWaking: "Night waking",
    feeding: "Feeding",
    nothingLogged: (isToday: boolean): string =>
      isToday ? "Nothing logged yet today." : "Nothing logged this day.",
  },
  sleepModal: {
    editTitle: "Edit sleep",
    logTitle: "Log sleep",
    startedAt: "Started at",
    endedAt: "Ended at",
    leaveBlankHint: "Leave both blank if still asleep.",
    nightSleepCheckbox: "🌆 This was the night sleep (not a nap)",
    errorSave: "Couldn't save the sleep session. Try again.",
    errorDelete: "Couldn't delete the sleep session. Try again.",
    deleteSleepSession: "Delete sleep session",
    deleteConfirmQuestion: "Delete this sleep session?",
    deleting: "Deleting…",
  },
  feedingModal: {
    editTitle: "Edit feeding",
    logTitle: "Log a feeding",
    occurredAt: "Occurred at",
    amount: "Amount",
    amountPlaceholder: "e.g. 120",
    unit: "Unit",
    food: "Food",
    noSolidFoodsHint: "No solid foods yet — ",
    addInSettings: "add some in Settings",
    errorSave: "Couldn't save the feeding. Try again.",
    errorDelete: "Couldn't delete the feeding. Try again.",
    deleteFeeding: "Delete feeding",
    deleteConfirmQuestion: "Delete this feeding?",
    deleting: "Deleting…",
  },
  nightWakingModal: {
    editTitle: "Edit night waking",
    logTitle: "Log a night waking",
    wokeAt: "Woke at",
    backAsleepAt: "Back asleep at",
    leaveBlankHint: "Leave both blank if he's still awake.",
    errorSave: "Couldn't save the night waking. Try again.",
    errorDelete: "Couldn't delete the night waking. Try again.",
    deleteWaking: "Delete night waking",
    deleteConfirmQuestion: "Delete this night waking?",
    deleting: "Deleting…",
  },
  feedTypes: {
    bottle: "Bottle",
    breast: "Breast",
    formula: "Formula",
    solid: "Solid",
  },
  settings: {
    title: "Settings",
    account: "Account",
    signedInAs: (name: string) => `Signed in as ${name}`,
    signOut: "Sign out",
    appearance: "Appearance",
    light: "Light",
    dark: "Dark",
    language: "Language",
    adminOnly: "Only Edin and Amina can change this.",
    solidFoods: "Solid foods",
    foodPlaceholder: "e.g. Banana",
    add: "Add",
    removeFoodAria: (name: string) => `Remove ${name}`,
    foodAlreadyExists: "That food is already on the list.",
    foodAddError: "Couldn't add that food.",
    solidFoodsHint: "These show up as options when logging a solid feeding.",
    notifications: "Notifications",
    notificationsHint:
      "Ten minutes before a nap or bedtime is due, five minutes before a nap should end, and an hour and a half after each feeding while he's awake. Each device is separate.",
    notificationsOn: "On for this device",
    enableNotifications: "Turn on for this device",
    disableNotifications: "Turn off",
    sendTestNotification: "Send a test",
    testNotificationSent: "Sent — check your notifications.",
    notificationsDenied:
      "Notifications are blocked for Gicko. Allow them in your browser or system settings first.",
    notificationsUnsupported: "This browser can't show notifications.",
    notificationsNeedsInstall:
      "On iPhone, add Gicko to your home screen first: tap Share, then \u201cAdd to Home Screen\u201d, and open it from there.",
    notificationsError: "That didn't work. Try again.",
    wakeWindows: "Wake windows",
    wakeWindowPlaceholder: "e.g. 3.5",
    removeWakeWindowAria: (n: number) => `Remove wake window ${n}`,
    wakeWindowsHint:
      "How long he should stay awake before each nap, in order. They add up to the day's total awake time: if one stretch runs short or long, the windows left adjust to keep that total. The last value repeats for any later naps.",
    feedingInterval: "Feeding interval",
    feedingIntervalPlaceholder: "e.g. 1.5",
    feedingIntervalHint:
      "How long after a feeding the next one is due, in hours. Used for the feeding reminder.",
    save: "Save",
    saved: "Saved",
    napDurations: "Nap lengths",
    napDurationPlaceholder: "e.g. 1.5",
    removeNapDurationAria: (n: number) => `Remove nap length ${n}`,
    napDurationsHint:
      "How long each nap should last, in order. They add up to the day's total nap time: overshoot one and the naps left shorten, wake early and they lengthen. The last value repeats for any later naps.",
    exportData: "Export data",
    from: "From",
    to: "To",
    exportError: "Couldn't fetch data for export.",
    preparing: "Preparing…",
    exportToExcel: "Export to Excel",
    exportHint: "Downloads an .xlsx with separate Sleep and Feedings sheets for the selected range.",
  },
  login: {
    heading: "Gicko",
    subheading: "Sign in to keep tracking",
    username: "Username",
    password: "Password",
    incorrectCredentials: "Incorrect username or password.",
    signingIn: "Signing in…",
    signIn: "Sign in",
    staySignedIn: "You'll stay signed in on this device.",
  },
};

export type Translations = typeof en;

const bs: Translations = {
  nav: {
    home: "Početna",
    calendar: "Kalendar",
    dashboard: "Statistika",
    settings: "Postavke",
  },
  common: {
    cancel: "Otkaži",
    save: "Sačuvaj",
    saving: "Čuvanje…",
    loading: "Učitavanje…",
    no: "Ne",
    yesDelete: "Da, obriši",
    notesOptional: "Napomene (opcionalno)",
    none: "Ništa",
    previous: "Prethodno",
    next: "Sljedeće",
    today: "Danas",
  },
  actions: {
    logSleep: "😴 Zabilježi san",
    logFeeding: "🍼 Zabilježi hranjenje",
    logNightWaking: "🌙 Zabilježi buđenje",
  },
  home: {
    previousDay: "Prethodni dan",
    nextDay: "Sljedeći dan",
    today: "Danas",
    asleepSince: "Spava od",
    awakeSince: "Budan od",
    asleep: "u snu",
    awake: "budan",
    nightAwakening: "🌙 Noćno buđenje",
    wokeUp: "😴 Probudio se",
    morningWake: "☀️ Jutarnje buđenje",
    putDownToSleep: "🌙 Uspavaj",
    logAFeeding: "🍼 Zabilježi hranjenje",
    statNightSleep: "Noćni san",
    statDaytimeAwake: "Budnost tokom dana",
    statNapsTotal: "Ukupno dremki",
    statFeedings: "Hranjenja",
    statEaten: "Pojedeno",
    statNightWakeUps: "Noćna buđenja",
    statPoops: "Kakice",
    timeline: "Vremenska linija",
    feedingsToday: "Današnja hranjenja",
    noFeedingsToday: "Danas nema zabilježenih hranjenja.",
    wakeUpsToday: "Današnja noćna buđenja",
    noWakeUpsToday: "Danas nema noćnih buđenja.",
    awakeFor: (d: string) => `Budan ${d}`,
    napsToday: "Današnje dremke",
    noNapsToday: "Danas nema zabilježenih dremki.",
    logAPoop: "💩 Kakica",
    poopsToday: "Današnje kakice",
    noPoopsToday: "Danas nema zabilježenih kakica.",
    addPoop: "Dodaj jednu",
    removePoop: "Ukloni jednu",
    ongoing: "u toku",
    wakePromptQuestion: "Je li jeo tokom tog buđenja?",
    logIt: "Zabilježi",
    nextNap: "Sljedeća dremka",
    nextBedtime: "Sljedeće spavanje",
    expectedWakeUp: "Očekivano buđenje",
  },
  calendar: {
    month: "Mjesec",
    week: "Sedmica",
    weekdayLetters: ["P", "U", "S", "Č", "P", "S", "N"],
  },
  reminders: {
    nap_due: { title: "Dremka za 10 minuta", body: "Prozor budnosti je skoro pri kraju." },
    bedtime_due: { title: "Vrijeme za kupanje", body: "Spavanje je za dvadesetak minuta." },
    nap_end: { title: "Probudi ga za 5 minuta", body: "Dremka je skoro dostigla predviđenu dužinu." },
    feeding_due: { title: "Vrijeme za hranjenje", body: "Sat i po od zadnjeg hranjenja." },
    dismiss: "Sakrij",
  },
  activity: {
    title: "Nedavno",
    open: "Šta su drugi zabilježili",
    empty: "Niko drugi danas nije ništa zabilježio.",
    nap: "Dremka",
    nightSleep: "Noćni san",
    nightWaking: "Noćno buđenje",
    poop: "Kakica",
    markAllRead: "Označi sve kao pročitano",
  },
  dashboard: {
    title: "Statistika",
    napsCount: "Broj dremki",
    wakeUpLength: "Prosječno trajanje buđenja",
    feedingsMl: "Ukupno ml pojedeno",
    average: "Prosjek",
    noData: "Nema podataka u ovom periodu.",
  },
  dayDetail: {
    note: "Bilješka",
    notePlaceholder: "Nešto vrijedno pamćenja o ovom danu…",
  },
  timelineView: {
    nightSleep: "Noćni san",
    nap: "Dremka",
    nightWaking: "Noćno buđenje",
    feeding: "Hranjenje",
    nothingLogged: (isToday: boolean) =>
      isToday ? "Danas još ništa nije zabilježeno." : "Ovaj dan nema zabilježenog.",
  },
  sleepModal: {
    editTitle: "Uredi san",
    logTitle: "Zabilježi san",
    startedAt: "Početak",
    endedAt: "Kraj",
    leaveBlankHint: "Ostavite oba prazna ako još spava.",
    nightSleepCheckbox: "🌆 Ovo je bio noćni san (nije dremka)",
    errorSave: "Nije moguće sačuvati san. Pokušajte ponovo.",
    errorDelete: "Nije moguće obrisati san. Pokušajte ponovo.",
    deleteSleepSession: "Obriši san",
    deleteConfirmQuestion: "Obrisati ovaj san?",
    deleting: "Brisanje…",
  },
  feedingModal: {
    editTitle: "Uredi hranjenje",
    logTitle: "Zabilježi hranjenje",
    occurredAt: "Vrijeme",
    amount: "Količina",
    amountPlaceholder: "npr. 120",
    unit: "Jedinica",
    food: "Hrana",
    noSolidFoodsHint: "Još nema unesene hrane — ",
    addInSettings: "dodajte je u Postavkama",
    errorSave: "Nije moguće sačuvati hranjenje. Pokušajte ponovo.",
    errorDelete: "Nije moguće obrisati hranjenje. Pokušajte ponovo.",
    deleteFeeding: "Obriši hranjenje",
    deleteConfirmQuestion: "Obrisati ovo hranjenje?",
    deleting: "Brisanje…",
  },
  nightWakingModal: {
    editTitle: "Uredi noćno buđenje",
    logTitle: "Zabilježi noćno buđenje",
    wokeAt: "Probudio se u",
    backAsleepAt: "Ponovo zaspao u",
    leaveBlankHint: "Ostavite oba prazna ako je još budan.",
    errorSave: "Nije moguće sačuvati noćno buđenje. Pokušajte ponovo.",
    errorDelete: "Nije moguće obrisati noćno buđenje. Pokušajte ponovo.",
    deleteWaking: "Obriši noćno buđenje",
    deleteConfirmQuestion: "Obrisati ovo noćno buđenje?",
    deleting: "Brisanje…",
  },
  feedTypes: {
    bottle: "Bočica",
    breast: "Dojenje",
    formula: "Formula",
    solid: "Kruta hrana",
  },
  settings: {
    title: "Postavke",
    account: "Nalog",
    signedInAs: (name: string) => `Prijavljeni kao ${name}`,
    signOut: "Odjava",
    appearance: "Izgled",
    light: "Svijetlo",
    dark: "Tamno",
    language: "Jezik",
    adminOnly: "Samo Edin i Amina mogu ovo mijenjati.",
    solidFoods: "Kruta hrana",
    foodPlaceholder: "npr. Banana",
    add: "Dodaj",
    removeFoodAria: (name: string) => `Ukloni ${name}`,
    foodAlreadyExists: "Ta namirnica je već na listi.",
    foodAddError: "Nije moguće dodati namirnicu.",
    solidFoodsHint: "Ove namirnice se pojavljuju kao opcije prilikom bilježenja krute hrane.",
    notifications: "Obavještenja",
    notificationsHint:
      "Deset minuta prije dremke ili spavanja, pet minuta prije nego dremka treba završiti, i sat i po nakon svakog hranjenja dok je budan. Svaki uređaj se uključuje posebno.",
    notificationsOn: "Uključeno na ovom uređaju",
    enableNotifications: "Uključi na ovom uređaju",
    disableNotifications: "Isključi",
    sendTestNotification: "Pošalji test",
    testNotificationSent: "Poslano — provjeri obavještenja.",
    notificationsDenied:
      "Obavještenja su blokirana za Gicko. Dozvoli ih u postavkama pregledača ili sistema.",
    notificationsUnsupported: "Ovaj pregledač ne podržava obavještenja.",
    notificationsNeedsInstall:
      "Na iPhoneu prvo dodaj Gicko na početni ekran: dodirni Podijeli, pa \u201cDodaj na početni ekran\u201d, i otvori ga odatle.",
    notificationsError: "Nije uspjelo. Pokušaj ponovo.",
    wakeWindows: "Prozori budnosti",
    wakeWindowPlaceholder: "npr. 3.5",
    removeWakeWindowAria: (n: number) => `Ukloni prozor budnosti ${n}`,
    wakeWindowsHint:
      "Koliko dugo treba biti budan prije svake dremke, redoslijedom. Zbir je ukupno vrijeme budnosti u danu: ako jedan period bude kraći ili duži, preostali prozori se prilagode da zbir ostane isti. Posljednja vrijednost se ponavlja za sve naredne dremke.",
    feedingInterval: "Interval hranjenja",
    feedingIntervalPlaceholder: "npr. 1.5",
    feedingIntervalHint:
      "Koliko sati nakon hranjenja slijedi sljedeće. Koristi se za podsjetnik o hranjenju.",
    save: "Sačuvaj",
    saved: "Sačuvano",
    napDurations: "Dužina dremki",
    napDurationPlaceholder: "npr. 1.5",
    removeNapDurationAria: (n: number) => `Ukloni dužinu dremke ${n}`,
    napDurationsHint:
      "Koliko dugo treba trajati svaka dremka, redoslijedom. Zbir je ukupno vrijeme dremki u danu: ako jedna potraje duže, preostale se skrate, a ako se probudi ranije, produže se. Posljednja vrijednost se ponavlja za sve naredne dremke.",
    exportData: "Izvoz podataka",
    from: "Od",
    to: "Do",
    exportError: "Nije moguće preuzeti podatke za izvoz.",
    preparing: "Priprema…",
    exportToExcel: "Izvezi u Excel",
    exportHint: "Preuzima .xlsx datoteku sa odvojenim listovima za san i hranjenja za odabrani period.",
  },
  login: {
    heading: "Gicko",
    subheading: "Prijavite se da nastavite praćenje",
    username: "Korisničko ime",
    password: "Lozinka",
    incorrectCredentials: "Pogrešno korisničko ime ili lozinka.",
    signingIn: "Prijavljivanje…",
    signIn: "Prijavi se",
    staySignedIn: "Ostat ćete prijavljeni na ovom uređaju.",
  },
};

const de: Translations = {
  nav: {
    home: "Start",
    calendar: "Kalender",
    dashboard: "Statistiken",
    settings: "Einstellungen",
  },
  common: {
    cancel: "Abbrechen",
    save: "Speichern",
    saving: "Speichern…",
    loading: "Wird geladen…",
    no: "Nein",
    yesDelete: "Ja, löschen",
    notesOptional: "Notizen (optional)",
    none: "Keine",
    previous: "Zurück",
    next: "Weiter",
    today: "Heute",
  },
  actions: {
    logSleep: "😴 Schlaf erfassen",
    logFeeding: "🍼 Fütterung erfassen",
    logNightWaking: "🌙 Aufwachen erfassen",
  },
  home: {
    previousDay: "Vorheriger Tag",
    nextDay: "Nächster Tag",
    today: "Heute",
    asleepSince: "Schläft seit",
    awakeSince: "Wach seit",
    asleep: "schlafend",
    awake: "wach",
    nightAwakening: "🌙 Nächtliches Aufwachen",
    wokeUp: "😴 Aufgewacht",
    morningWake: "☀️ Morgendliches Aufwachen",
    putDownToSleep: "🌙 Hinlegen",
    logAFeeding: "🍼 Fütterung erfassen",
    statNightSleep: "Nachtschlaf",
    statDaytimeAwake: "Wachzeit tagsüber",
    statNapsTotal: "Nickerchen gesamt",
    statFeedings: "Fütterungen",
    statEaten: "Gegessen",
    statNightWakeUps: "Nächtliches Aufwachen",
    statPoops: "Kacka",
    timeline: "Zeitleiste",
    feedingsToday: "Heutige Fütterungen",
    noFeedingsToday: "Heute keine Fütterungen erfasst.",
    wakeUpsToday: "Heutiges nächtliches Aufwachen",
    noWakeUpsToday: "Heute kein nächtliches Aufwachen.",
    awakeFor: (d: string) => `${d} wach`,
    napsToday: "Heutige Nickerchen",
    noNapsToday: "Heute keine Nickerchen erfasst.",
    logAPoop: "💩 Kacka",
    poopsToday: "Heutige Kacka",
    noPoopsToday: "Heute keine Kacka erfasst.",
    addPoop: "Eine hinzufügen",
    removePoop: "Eine entfernen",
    ongoing: "andauernd",
    wakePromptQuestion: "Hat er während des Aufwachens gegessen?",
    logIt: "Erfassen",
    nextNap: "Nächstes Nickerchen",
    nextBedtime: "Nächste Schlafenszeit",
    expectedWakeUp: "Erwartetes Aufwachen",
  },
  calendar: {
    month: "Monat",
    week: "Woche",
    weekdayLetters: ["M", "D", "M", "D", "F", "S", "S"],
  },
  reminders: {
    nap_due: { title: "Nickerchen in 10 Minuten", body: "Das Wachfenster ist fast vorbei." },
    bedtime_due: { title: "Zeit für das Bad", body: "In gut zwanzig Minuten ist Schlafenszeit." },
    nap_end: { title: "Weck ihn in 5 Minuten", body: "Das Nickerchen hat fast seine geplante Länge." },
    feeding_due: { title: "Zeit für eine Mahlzeit", body: "Eineinhalb Stunden seit der letzten." },
    dismiss: "Ausblenden",
  },
  activity: {
    title: "Zuletzt",
    open: "Was die anderen erfasst haben",
    empty: "Heute hat sonst niemand etwas erfasst.",
    nap: "Nickerchen",
    nightSleep: "Nachtschlaf",
    nightWaking: "Nächtliches Aufwachen",
    poop: "Kacka",
    markAllRead: "Alle als gelesen markieren",
  },
  dashboard: {
    title: "Statistiken",
    napsCount: "Anzahl Nickerchen",
    wakeUpLength: "Ø Dauer nächtliches Aufwachen",
    feedingsMl: "Gesamt ml gegessen",
    average: "Ø",
    noData: "Keine Daten in diesem Zeitraum.",
  },
  dayDetail: {
    note: "Notiz",
    notePlaceholder: "Etwas Erinnerungswertes über diesen Tag…",
  },
  timelineView: {
    nightSleep: "Nachtschlaf",
    nap: "Nickerchen",
    nightWaking: "Nächtliches Aufwachen",
    feeding: "Fütterung",
    nothingLogged: (isToday: boolean) =>
      isToday ? "Heute noch nichts erfasst." : "An diesem Tag nichts erfasst.",
  },
  sleepModal: {
    editTitle: "Schlaf bearbeiten",
    logTitle: "Schlaf erfassen",
    startedAt: "Begonnen um",
    endedAt: "Beendet um",
    leaveBlankHint: "Beide leer lassen, falls noch schlafend.",
    nightSleepCheckbox: "🌆 Das war der Nachtschlaf (kein Nickerchen)",
    errorSave: "Schlaf konnte nicht gespeichert werden. Bitte erneut versuchen.",
    errorDelete: "Schlaf konnte nicht gelöscht werden. Bitte erneut versuchen.",
    deleteSleepSession: "Schlaf löschen",
    deleteConfirmQuestion: "Diesen Schlaf löschen?",
    deleting: "Löschen…",
  },
  feedingModal: {
    editTitle: "Fütterung bearbeiten",
    logTitle: "Fütterung erfassen",
    occurredAt: "Zeitpunkt",
    amount: "Menge",
    amountPlaceholder: "z. B. 120",
    unit: "Einheit",
    food: "Nahrung",
    noSolidFoodsHint: "Noch keine feste Nahrung — ",
    addInSettings: "in den Einstellungen hinzufügen",
    errorSave: "Fütterung konnte nicht gespeichert werden. Bitte erneut versuchen.",
    errorDelete: "Fütterung konnte nicht gelöscht werden. Bitte erneut versuchen.",
    deleteFeeding: "Fütterung löschen",
    deleteConfirmQuestion: "Diese Fütterung löschen?",
    deleting: "Löschen…",
  },
  nightWakingModal: {
    editTitle: "Nächtliches Aufwachen bearbeiten",
    logTitle: "Nächtliches Aufwachen erfassen",
    wokeAt: "Aufgewacht um",
    backAsleepAt: "Wieder eingeschlafen um",
    leaveBlankHint: "Beide leer lassen, falls er noch wach ist.",
    errorSave: "Nächtliches Aufwachen konnte nicht gespeichert werden. Bitte erneut versuchen.",
    errorDelete: "Nächtliches Aufwachen konnte nicht gelöscht werden. Bitte erneut versuchen.",
    deleteWaking: "Nächtliches Aufwachen löschen",
    deleteConfirmQuestion: "Dieses nächtliche Aufwachen löschen?",
    deleting: "Löschen…",
  },
  feedTypes: {
    bottle: "Fläschchen",
    breast: "Stillen",
    formula: "Fertignahrung",
    solid: "Feste Nahrung",
  },
  settings: {
    title: "Einstellungen",
    account: "Konto",
    signedInAs: (name: string) => `Angemeldet als ${name}`,
    signOut: "Abmelden",
    appearance: "Erscheinungsbild",
    light: "Hell",
    dark: "Dunkel",
    language: "Sprache",
    adminOnly: "Nur Edin und Amina können das ändern.",
    solidFoods: "Feste Nahrung",
    foodPlaceholder: "z. B. Banane",
    add: "Hinzufügen",
    removeFoodAria: (name: string) => `${name} entfernen`,
    foodAlreadyExists: "Dieses Lebensmittel ist bereits auf der Liste.",
    foodAddError: "Lebensmittel konnte nicht hinzugefügt werden.",
    solidFoodsHint: "Diese erscheinen als Optionen beim Erfassen einer Fütterung mit fester Nahrung.",
    notifications: "Benachrichtigungen",
    notificationsHint:
      "Zehn Minuten vor einem Nickerchen oder der Schlafenszeit, fünf Minuten bevor ein Nickerchen enden soll, und eineinhalb Stunden nach jeder Mahlzeit, solange er wach ist. Jedes Gerät wird einzeln aktiviert.",
    notificationsOn: "Auf diesem Gerät aktiv",
    enableNotifications: "Auf diesem Gerät aktivieren",
    disableNotifications: "Deaktivieren",
    sendTestNotification: "Test senden",
    testNotificationSent: "Gesendet — schau in deine Benachrichtigungen.",
    notificationsDenied:
      "Benachrichtigungen sind für Gicko blockiert. Erlaube sie in den Browser- oder Systemeinstellungen.",
    notificationsUnsupported: "Dieser Browser kann keine Benachrichtigungen anzeigen.",
    notificationsNeedsInstall:
      "Auf dem iPhone zuerst Gicko zum Home-Bildschirm hinzufügen: auf Teilen tippen, dann \u201cZum Home-Bildschirm\u201d, und von dort öffnen.",
    notificationsError: "Hat nicht funktioniert. Versuche es erneut.",
    wakeWindows: "Wachfenster",
    wakeWindowPlaceholder: "z. B. 3.5",
    removeWakeWindowAria: (n: number) => `Wachfenster ${n} entfernen`,
    wakeWindowsHint:
      "Wie lange er vor jedem Nickerchen wach bleiben soll, der Reihe nach. Zusammen ergeben sie die gesamte Wachzeit des Tages: fällt eine Phase kürzer oder länger aus, passen sich die verbleibenden Fenster an, damit die Summe stimmt. Der letzte Wert wiederholt sich für alle weiteren Nickerchen.",
    feedingInterval: "Fütterungsabstand",
    feedingIntervalPlaceholder: "z. B. 1.5",
    feedingIntervalHint:
      "Wie viele Stunden nach einer Mahlzeit die nächste fällig ist. Wird für die Erinnerung verwendet.",
    save: "Speichern",
    saved: "Gespeichert",
    napDurations: "Nickerchen-Dauer",
    napDurationPlaceholder: "z. B. 1.5",
    removeNapDurationAria: (n: number) => `Nickerchen-Dauer ${n} entfernen`,
    napDurationsHint:
      "Wie lange jedes Nickerchen dauern soll, der Reihe nach. Zusammen ergeben sie die gesamte Nickerchen-Zeit des Tages: dauert eines länger, werden die übrigen kürzer, wacht er früher auf, werden sie länger. Der letzte Wert wiederholt sich für alle weiteren Nickerchen.",
    exportData: "Daten exportieren",
    from: "Von",
    to: "Bis",
    exportError: "Daten für den Export konnten nicht geladen werden.",
    preparing: "Wird vorbereitet…",
    exportToExcel: "Nach Excel exportieren",
    exportHint:
      "Lädt eine .xlsx-Datei mit getrennten Blättern für Schlaf und Fütterungen für den gewählten Zeitraum herunter.",
  },
  login: {
    heading: "Gicko",
    subheading: "Melde dich an, um weiter zu tracken",
    username: "Benutzername",
    password: "Passwort",
    incorrectCredentials: "Falscher Benutzername oder falsches Passwort.",
    signingIn: "Anmelden…",
    signIn: "Anmelden",
    staySignedIn: "Du bleibst auf diesem Gerät angemeldet.",
  },
};

export const translations: Record<Language, Translations> = { en, bs, de };
